...
In addition, we should also consider the convergence of the variance of the noise amount. (It's easy to see that it converges when $\epsilon' > \beta \cdot (d+2)$; however, we have not yet discussed for the case where $\epsilon' \leq \beta \cdot (d+2)$. Therefore, in fact, any results in the paper where $\epsilon' \leq \beta \cdot (d+2)$ should be regarded as merely a reference. (That said, our SIDs are still more recommendable than existing ones.))
